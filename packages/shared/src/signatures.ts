/**
 * HATP batch signatures — Ed25519 provenance.
 *
 * Every HATP batch SHOULD be signed by the emitting Scout or Node.
 ...
 *   `${HATP_VERSION}.${schema_hash}.${sha256(events_canonical)}`
 *
...
import { HATP_VERSION, HATP_SCHEMA_HASH, canonicalize, type HATPEvent } from './hatp.js'

export interface HATPKeypair {
...
export interface SignedBatchEnvelope {
  hatp_version: typeof HATP_VERSION
  schema_hash: typeof HATP_SCHEMA_HASH
 ...
 */
export function generateSigningKeypair(): HATPKeypair {
 ...
 */
export function eventsDigest(events: HATPEvent[]): string {
...
function signingSurface(digest: string): Buffer {
  return Buffer.from(`${HATP_VERSION}.${HATP_SCHEMA_HASH}.${digest}`, 'utf8')
}

/**
 * Sign a batch of HATP events with an Ed25519 private key (PKCS8 PEM).
 */
export function signBatch(events: HATPEvent[], keypair: HATPKeypair): SignedBatchEnvelope {
...
export function verifyBatch(
  ...
  if (envelope.hatp_version !== HATP_VERSION) return false
  if (envelope.schema_hash !== HATP_SCHEMA_HASH) return false
...
export function publishKey(keypair: HATPKeypair, ttlDays = 180): PublishedKey {
  const now = Date.now()
  return {
    kid: keypair.kid,
    alg: 'EdDSA',
