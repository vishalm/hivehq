...
Every TTP event from a Scout must carry a valid `scout_cert` issued by a Node Hub CA. The Node Hub CA is issued by the Hive constellation during org enrollment.
...
2. Hive compares declared token volume (from TTP events) vs billed tokens (from provider billing API)
