# Protocol Versioning
### HATP Version Negotiation · Breaking Change Policy · Migration · Compatibility Matrix

> A protocol that breaks its implementers is not a protocol — it is a liability. This document specifies exactly what HATP promises, how versions negotiate, and how schema evolution happens without breaking existing deployments.

---

## Version Semantics

HATP uses semantic versioning with specific meanings at each position:

```
HATP / MAJOR . MINOR . PATCH
...
**Current version: HATP/0.1**

During `0.x` pre-stable phase: MINOR increments may include breaking changes. This is explicitly communicated as pre-stable behaviour. Stability guarantees begin at `HATP/1.0`.
...
Scout includes in enrollment request:
  X-HATP-Version: 0.1
  X-HATP-Capabilities: batch,streaming,msgpack

Node Hub responds:
  X-HATP-Version-Accepted: 0.1
  X-HATP-Capabilities-Accepted: batch,msgpack
  X-HATP-Min-Version: 0.1
  X-HATP-Max-Version: 0.2    (when 0.2 is released)
```
...
Response (version too old — hard reject):
  400 Bad Request
  X-HATP-Min-Version: 0.2
  X-HATP-Upgrade-Guide: https://hive.io/docs/hatp-upgrade/0.1-to-0.2

Response (version too new — soft reject):
  200 OK (events accepted)
  X-HATP-Version-Mismatch: true
  X-HATP-Max-Supported: 0.1
  X-HATP-Unknown-Fields-Ignored: governance.new_field_xyz
```
...
**DegradedMode:** Scout sends events with new fields that Node doesn't understand. Node accepts the events, logs unknown fields, strips them before storage. Scout is notified via `X-HATP-Unknown-Fields-Ignored` header. Data is not lost — it is downgraded.
...
- Removing a field from `HATPEvent`
...
```mermaid
timeline
    title HATP Version Migration Lifecycle
    section Announcement
        T-90 days : HATP/X.0 draft published for comment
                  : RFC open · 30-day comment period
    section Preview
        T-60 days : HATP/X.0 release candidate published
                  : HIVE Scout updated to support both old and new
                  : Node Hub supports both in parallel (negotiation)
    section Release
        T-0       : HATP/X.0 stable released
...
```bash
# Migrate stored events from HATP/0.1 to HATP/1.0 format
npx @hatp/migrate --from 0.1 --to 1.0 \
...
All stored HATP events retain their `hatp_version` field. The database never silently upgrades stored events. This means:
...
- Reports can specify which version's semantics to use: `?hatp_version=0.1`

---

## Backward Compatibility Guarantees (post HATP/1.0)

Once `HATP/1.0` is released, the HATP Working Group commits to:
...
|---|---|---|---|
| HATP/0.1 | HATP/0.1 | HATP/0.1 | Fully compatible |
| HATP/0.1 | HATP/0.2 | HATP/0.2 | Scout in DegradedMode — new fields ignored |
| HATP/0.2 | HATP/0.1 | HATP/0.1 | Scout downgraded to 0.1 at handshake |
| HATP/0.1 | HATP/1.0 | HATP/1.0 | Rejected — update required (breaking change) |

Post-1.0 guarantee (2-version window):

| Scout version | Node Hub version | Status |
|---|---|---|
| HATP/1.x | HATP/1.x | Fully compatible |
| HATP/1.x | HATP/2.x | DegradedMode (new fields ignored) |
| HATP/2.x | HATP/1.x | Downgraded at handshake |
| HATP/1.x | HATP/3.x | Rejected — 2 major versions behind |

---

## Unknown Field Handling — Required for All Implementations

Any HATP implementation MUST silently ignore unknown fields in a received `HATPEvent`. This is what allows forward compatibility (older Node reading newer Scout events).

```typescript
// Required behaviour in any compliant parser
function parseHATPEvent(raw: unknown): HATPEvent {
  // Strip unknown fields — do NOT throw on them
  const known = pickKnownFields(raw, HATP_0_1_FIELDS)
  // Validate known fields strictly
  return validate(known)
}
```

A Node Hub that throws `UnknownFieldError` on receiving a newer Scout's event is **not HATP-compliant**.

---

*See also: [HATP Protocol](./protocol.md) · [Architecture](./architecture.md) · [Build Sequence](./build-sequence.md)*
