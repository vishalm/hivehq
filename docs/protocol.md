# HATP — AI Industry Telemetry Protocol
### The open standard for AI consumption telemetry · v0.1 · Governed by HIVE

> **HATP is to AI consumption what OpenTelemetry is to infrastructure observability.**
...
```mermaid
graph LR
    subgraph BEFORE ["Without HATP — Today"]
        OAI_B["OpenAI\nCustom format"]
        ANT_B["Anthropic\nDifferent format"]
        AZR_B["Azure OAI\nAnother format"]
        GEM_B["Gemini\nYet another"]
    ...
    subgraph AFTER ["With HATP — Protocol-native"]
        HATP_CORE["HATP Event\nOne schema\nSigned · Versioned\nGovernance-native"]
        ANY["Any provider\nAny tool\nAny org"]
        COMP["Compliance-ready\nAudit trail\nData residency enforced"]
    end

    BEFORE -->|"HATP layer"| AFTER

    style BEFORE fill:#FFE8E8,stroke:#FF3B30,color:#1D1D1F
    style AFTER fill:#E8F9F0,stroke:#34C759,color:#1D1D1F
    style HATP_CORE fill:#007AFF,color:#fff,stroke:none
```
...
```mermaid
graph TB
    subgraph HATP_STACK ["HATP Protocol Stack"]
        direction TB
    ...
    style HATP_STACK fill:#F0EEFF,stroke:#5856D6,color:#1D1D1F
    style IMPL fill:#E8F4FF,stroke:#007AFF,color:#1D1D1F
```

---

## The Canonical Event — `HATPEvent v0.1`
...
interface HATPEvent {
  ...
  // ── Protocol metadata ─────────────────────────────────────────────────────
  hatp_version:    '0.1'                    // semver · breaking changes increment major
...
HATP maintains a canonical registry of provider identifiers. This ensures `provider: 'openai'` means the same thing in every implementation, everywhere.

```mermaid
graph TD
    subgraph REG ["HATP Provider Registry — Open · Community-maintained"]
        direction LR
...
Registry lives at: `github.com/hatp-protocol/registry` (to be established)

---

## Token Estimation Algorithm

When a provider does not expose token counts in response headers, HATP specifies a deterministic estimation algorithm so all implementations produce comparable numbers.
...
This ensures that telemetry aggregated across providers is comparable — a token is a token in HATP, regardless of provider.

---

## Governance Layer

**Governance is not optional in HATP. Every event carries a `GovernanceBlock`.**

This is the design principle that differentiates HATP from generic observability protocols. The GovernanceBlock is validated at ingest — a Node Hub or HIVE constellation will reject events without it.
...
sequenceDiagram
    ...
    SC->>ND: HATPEvent (with GovernanceBlock)
...
```
POST https://node.your-org.internal/api/v1/hatp/ingest
Content-Type: application/json          (or application/msgpack for binary)
Authorization: Bearer <scout_token>
X-HATP-Version: 0.1
X-HATP-Batch-ID: <uuid>

Body: HATPEvent[]                       (array · 1–500 events per request)
```
...
- On-demand: `POST /api/v1/hatp/flush` forces immediate drain
...
→ Server-Sent Events, one HATPEvent per line
...
### `@hatp/sdk` (TypeScript reference)

```typescript
import { HATPCollector } from '@hatp/sdk'

const collector = new HATPCollector({
  endpoint: 'https://node.your-org.internal/api/v1/hatp/ingest',
  token: process.env.HATP_TOKEN,
  governance: {
...
// HATPEvent emitted automatically. response is unchanged.
```
...
## HATP vs OpenTelemetry

HATP is not a replacement for OpenTelemetry. It is a complement — specifically designed for the AI consumption layer.

| Dimension | OpenTelemetry | HATP |
|---|---|---|
...
**You can run both.** OTel for infra. HATP for AI consumption. They are orthogonal.

---

## Governance Structure

HATP is an open standard. HIVE is the founding implementer and convenes the working group.

```mermaid
graph TD
    WG["HATP Working Group\nOpen membership · quarterly cadence"]
    ...
    subgraph PROCESS ["Change Process"]
        RFC["RFC submitted\nGitHub PR to hatp-protocol/spec"]
...
4. HIVE never requires HATP events to flow through HIVE infrastructure
...
For any org evaluating HATP + HIVE:

```
Auditor: "Show me your AI usage data."
You:     "Here's the HATP event log. Every event is signed, timestamped,
...
## Why HATP Is HIVE's Deepest Moat

The protocol is free and open. The reference network is HIVE.

If HATP becomes the industry standard:
- Every AI tool that implements HATP is, by default, HIVE-compatible
- Every compliance team that requires HATP is, by default, a HIVE lead
- Every government that mandates AI telemetry uses HATP — and HIVE is the platform

**HIVE does not need to own every deployment. HIVE needs to own the protocol.**

This is how standards-based businesses win: TCP/IP → Cisco. HTTP → Apache. OpenTelemetry → Datadog, Grafana, New Relic all integrated. HATP → HIVE.
