...
> The canonical `HATPEvent` covers the simple case: one request, one response. Reality is messier. This document specifies exactly how HATP handles every non-trivial LLM interaction pattern.
...
graph LR
    ...
    HATP["HATP event model\nmust handle all of these\ncorrectly and consistently"]
    PATTERNS --> HATP

    style HATP fill:#007AFF,color:#fff,stroke:none
```
...
// Event 2 with partial data
{
  ...
  status_code: 200,
  stream_cancelled: true,               // optional HATP extension field in v0.2
...
**In HATP/0.1:** `tool_use`, `tool_count`, `tool_result` are not in the schema. They are captured as follows:
...
The semantic "this was a tool call" is deferred to HATP/0.2. v0.1 captures the data accurately, just without the semantic flag.
...
**In HATP/0.1 practice:**
- Scout logs the full raw `payload_bytes` as a secondary metric (local only, not in HATP event)
- HATP event `payload_bytes` is the normalised text-only size
...
Scout (or `@hatp/sdk`) unwraps batch requests and emits individual HATPEvents per logical call:
...
**In HATP/0.1:** `batch_id` is not a schema field. Use `project_tag` or `dept_tag` to annotate batch jobs. The session_hash links request and response correctly.
...
```typescript
// @hatp/sdk assigns event_id at the time the original request is made
...
**What HIVE guarantees:** even in proxy mode, no user-identifiable information is in the HATP event. The `X-HIVE-User-Tag` must be a hash or opaque token — the org's responsibility to ensure.

**Phase 2 addition:** dedicated Proxy Scout mode that reads existing Kubernetes/NGINX auth headers and maps to HATP classification fields.
...
*See also: [HATP Protocol](./protocol.md) · [Protocol Versioning](./protocol-versioning.md) · [Data Model](./data-model.md)*
