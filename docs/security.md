...
**What an attacker gets if they intercept a HATP bundle:** encrypted telemetry with no API keys, no content, and a cert they cannot forge. Useless.
...
| Content exfiltration via connector | @hatp/sdk in app | Schema validator rejects non-schema fields · content_asserted: false enforced · see §6 | Medium — requires connector compromise |
...
The `@hatp/sdk` and all `@hive/connector-*` packages sit between user applications and AI providers. This is a privileged position.
...
| **Content-free assertion** | `HATPEvent.content_asserted: false` is a schema constant. Node Hub rejects any event with unexpected fields. A compromised connector cannot *store* content in HATP — it can only exfiltrate externally. |
...
| **Community connector policy** | Tier 2/3 connectors must pass HATP compliance test suite. Schema compliance verified by `@hatp/validator`. Non-compliant connectors rejected from registry. |
...
|---|---|---|---|
| `/api/v1/hatp/ingest` | 500 events / scout | 60 seconds | `429 Too Many Requests` + `Retry-After` header |
| `/api/v1/hatp/ingest` | 10,000 events / node | 60 seconds | `429` to all scouts on that node |
...
*See also: [Key Lifecycle](./key-lifecycle.md) · [Integrity](./integrity.md) · [HATP Protocol](./protocol.md) · [Architecture](./architecture.md)*
